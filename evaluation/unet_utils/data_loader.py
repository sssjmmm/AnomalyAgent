...
class MVTec_Anomaly_Detection(Dataset):
    def __init__(self, args,sample_name,length=5000,anomaly_id=None,recon=False):
        ...
        self.img_paths=[]
        self.mask_paths=[]
        # 获取mask目录名（默认为'mask'，可以通过args.mask_dir指定）
        mask_dir = getattr(args, 'mask_dir', 'mask')
        mask_ext = '.jpg'
        for idx,anomaly in enumerate(self.anomaly_names):
            img_path=[]
            mask_path=[]
            mask_dir_path = os.path.join(self.root_dir,anomaly,mask_dir)
            for i in range(min(len(os.listdir(mask_dir_path)),500)):
                img_path.append(os.path.join(self.root_dir,anomaly,'image','%d.jpg'%i))
                mask_path.append(os.path.join(self.root_dir,anomaly,mask_dir,'%d%s'%(i,mask_ext)))
